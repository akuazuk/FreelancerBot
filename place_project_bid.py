...
freelancer_oauth_token = "55TZA1eQgTxOtPmkiwCVgKrCR8AAHQ"
...
Please allow me to generate a few sample entries in order to gain your trust and satisfaction. I 
have reviewed your project requirements closely and can help you with this. We can discuss also the 
complexity of the project so I can provide you with a realistic ETA and feel free to contact me 
through chat to talk about your project in more detail. 
I have extensive knowledge of web scrapping with Python,scrapy, BeautifulSoup, Selenium. My 
extensive experience with web scraping using IP proxy rotation, multi thread and Bypassing captcha. 
You can see my past work on freelancer and on github.
...
while True:    
    ...
    if p:
        ...
        for x in p.get('projects'):
            ...
            amount = int((x.get('budget').get('maximum') - x.get('bid_stats').get('bid_avg'))/2) + 
x.get('bid_stats').get('bid_avg')
            
            try:
                bid = get_bids(session, **get_bids_data)
                if bid and x.get('status') == 'active' and (x.get('currency').get('code') == 'USD' 
or x.get('currency').get('code') == 'AUD'):
